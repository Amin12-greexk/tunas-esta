interface SectionSeparatorProps {
  variant?: "wave" | "diagonal" | "curve" | "zigzag" | "dots";
  color?: "gray" | "green" | "blue" | "gradient";
  direction?: "left" | "right";
  height?: "sm" | "md" | "lg";
}

export function SectionSeparator({ 
  variant = "wave", 
  color = "gray", 
  direction = "left",
  height = "md" 
}: SectionSeparatorProps) {
  
  const getHeightClass = () => {
    switch (height) {
      case "sm": return "h-8";
      case "md": return "h-16";
      case "lg": return "h-24";
      default: return "h-16";
    }
  };

  const getColorClasses = () => {
    switch (color) {
      case "gray": return "text-gray-200";
      case "green": return "text-green-100";
      case "blue": return "text-blue-100";
      case "gradient": return "text-gray-200";
      default: return "text-gray-200";
    }
  };

  const renderWave = () => (
    <div className={`w-full ${getHeightClass()} relative overflow-hidden`}>
      <svg
        className={`absolute inset-0 w-full h-full ${getColorClasses()}`}
        preserveAspectRatio="none"
        viewBox="0 0 1200 120"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M0,0V46.29c47.79,22.2,103.59,32.17,158,28,70.36-5.37,136.33-33.31,206.8-37.5C438.64,32.43,512.34,53.67,583,72.05c69.27,18,138.3,24.88,209.4,13.08,36.15-6,69.85-17.84,104.45-29.34C989.49,25,1113-14.29,1200,52.47V0Z"
          opacity="0.25"
          fill="currentColor"
        />
        <path
          d="M0,0V15.81C13,36.92,27.64,56.86,47.69,72.05,99.41,111.27,165,111,224.58,91.58c31.15-10.15,60.09-26.07,89.67-39.8,40.92-19,84.73-46,130.83-49.67,36.26-2.85,70.9,9.42,98.6,31.56,31.77,25.39,62.32,62,103.63,73,40.44,10.79,81.35-6.69,119.13-24.28s75.16-39,116.92-43.05c59.73-5.85,113.28,22.88,168.9,38.84,30.2,8.66,59,6.17,87.09-7.5,22.43-10.89,48-26.93,60.65-49.24V0Z"
          opacity="0.5"
          fill="currentColor"
        />
        <path
          d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z"
          fill="currentColor"
        />
      </svg>
    </div>
  );

  const renderDiagonal = () => (
    <div className={`w-full ${getHeightClass()} relative overflow-hidden`}>
      <svg
        className={`absolute inset-0 w-full h-full ${getColorClasses()}`}
        preserveAspectRatio="none"
        viewBox="0 0 1200 120"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d={direction === "left" 
            ? "M0,0L1200,120L1200,0Z" 
            : "M0,120L1200,0L0,0Z"
          }
          fill="currentColor"
        />
      </svg>
    </div>
  );

  const renderCurve = () => (
    <div className={`w-full ${getHeightClass()} relative overflow-hidden`}>
      <svg
        className={`absolute inset-0 w-full h-full ${getColorClasses()}`}
        preserveAspectRatio="none"
        viewBox="0 0 1200 120"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M0,0Q600,120,1200,0L1200,0L0,0Z"
          fill="currentColor"
        />
      </svg>
    </div>
  );

  const renderZigzag = () => (
    <div className={`w-full ${getHeightClass()} relative overflow-hidden`}>
      <svg
        className={`absolute inset-0 w-full h-full ${getColorClasses()}`}
        preserveAspectRatio="none"
        viewBox="0 0 1200 120"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M0,0L200,120L400,0L600,120L800,0L1000,120L1200,0L1200,0L0,0Z"
          fill="currentColor"
        />
      </svg>
    </div>
  );

  const renderDots = () => (
    <div className={`w-full ${getHeightClass()} flex items-center justify-center`}>
      <div className="flex space-x-4">
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
            className={`w-3 h-3 rounded-full ${
              color === "gradient" 
                ? "bg-gradient-to-r from-green-400 to-blue-400" 
                : `bg-${color}-300`
            } animate-pulse`}
            style={{ animationDelay: `${i * 200}ms` }}
          />
        ))}
      </div>
    </div>
  );

  const renderSeparator = () => {
    switch (variant) {
      case "wave": return renderWave();
      case "diagonal": return renderDiagonal();
      case "curve": return renderCurve();
      case "zigzag": return renderZigzag();
      case "dots": return renderDots();
      default: return renderWave();
    }
  };

  return (
    <div className="relative">
      {color === "gradient" && (
        <div className="absolute inset-0 bg-gradient-to-r from-green-100 via-blue-100 to-purple-100 opacity-50" />
      )}
      {renderSeparator()}
    </div>
  );
}